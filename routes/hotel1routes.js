...
import hotel1UsersDetails from '../models/hotel1model.js';
...
router.post("/users/register", async (req, res) => {  
    ...
    const newUser = new hotel1UsersDetails({
      firstName: req.body.firstName,
      lastName: req.body.lastName,
      email: req.body.email,
      phoneNumber: req.body.phoneNumber,
      country: req.body.country,
      arrivalFrom:req.body.arrivalFrom,
      departTo:req.body.departTo,
    });
    try {
  ...
  router.get("/users/registeredusers", async (req, res) => {  
  
    try {
      const users = await hotel1UsersDetails.find().sort({_id:-1});
      res.json(users);
    } catch (err) {
      res.json({ message: err });
    //  res.send("not uploaded");
  ...
  router.patch("/users/register/frontIdImg/:id", async (req, res) => {  
    try {
      const updateRegister = await hotel1UsersDetails.updateOne({
  ...
  router.patch("/users/register/backIdImg/:id", async (req, res) => {  
  ...
  router.patch("/users/register/selfie/:id", async (req, res) => {  
    //console.log('updating')
    try {
      const updateRegister = await hotel1UsersDetails.updateOne({
    ...
    // console.log("called");
    //checking if the EMAIL EXIST/hoel
  //   try{
  //     const userExist = await hotel1UsersDetails.findById(req.params.id);
  //     res.send(userExist);
  ...
  router.post("/feedback/webhook",(req, res)=>{
    setTimeout(()=>{
     ...
     //   console.log(idNumber)
       ...
       //  console.log(onWatchList)
            ...
            //  console.log(`${host}/users/onboarding/register/selfie/62cff6a006dbfc968862c3ac`)
               req =  unirest('PATCH', `https://www.hotel.veriphy.co/users/hotel1/updatemanyregister/${userID}`)
               .headers({
                 'Content-Type': 'application/json'
               })
               .send(JSON.stringify({
